 ...
 *
 * Ported from doxa-cns / openclaw for doxa-browser-extension. .cjs extension
 * because this repo's package.json sets "type": "module" — plain .js files
 * are ES modules there and require() would throw.
 *
...
${c.bright}Commands:${c.reset}
  ...
  ${c.green}add <entity> <observation> <source-file> [line-number] [--type <EntityType>]${c.reset}
    ...
    Pass --type for NEW entities so reconcile records the right type
    (defaults to Unknown). Idempotent on re-runs.
...
function add(args) {
  ...
  // Guard: an entity name starting with '-' is almost always a mis-parsed CLI
  // flag landing in the positional <entity> slot. Reject at the point of
  // entry.
  ...
  // Ontology guard — two distinct drift sources, both closed here rather
  // than reported later by an audit tool nobody is blocked by:
  //   1. An INVENTED type — rejected, with the nearest legal kind named.
  //   2. A MISSING type on a NEW entity, which would silently become
  //      `Unknown` — refused.
  // Appending to an EXISTING entity may still omit --type: the entity
  // already has one, and demanding it again would break every legitimate
  // append.
  {
  ...
  // source-of-truth file so a future federation sync (if any) has it.
  // Idempotent.
