...
# Ported from doxa-cns / openclaw for doxa-browser-extension.
#
# A doxa-browser-extension PR merge — via mcp__github__merge_pull_request OR a Bash
...
if [ -n "$repo" ]; then
  ...
  if [ -n "$flag_repo" ]; then
    case "$flag_repo" in
      doxa-browser-extension|*/doxa-browser-extension) ;;
      *) exit 0 ;;
    esac
  else
...
# This repo's layout: sources are src/**.ts, tests are test/*.test.mjs run by
