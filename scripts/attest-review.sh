...
# openclaw): .claude/hooks/review-gate.sh blocks doxa-browser-extension PR merges
# with no attestation for a local branch tip, and .claude/hooks/test-gate.sh
...
# An option-looking argument is a mistake, not a verdict. Reject it loudly
# rather than banking a fat-fingered flag as a merge authorization.
case "$verdict" in
