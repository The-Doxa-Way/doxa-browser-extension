#!/usr/bin/env bash
#
# review-gate.sh — PreToolUse gate on landing a doxa-browser-extension PR merge.
...
# doxa-browser-extension PR merge — via mcp__github__merge_pull_request OR a Bash
...
# doxa-browser-extension, git lookups failing — allows the call. Only the confirmed
# case blocks: a doxa-browser-extension merge with no attestation matching any local
...
if [ -n "$repo" ]; then
  ...
  [ "$repo" = "doxa-browser-extension" ] || exit 0
  ...
  if [ -n "$flag_repo" ]; then
    case "$flag_repo" in
      doxa-browser-extension|*/doxa-browser-extension) ;;   # gate
...

{
  ...
  echo "Every doxa-browser-extension chunk merges only after its INDEPENDENT review pass (CLAUDE.md Landing doctrine — self-review never counts)."
