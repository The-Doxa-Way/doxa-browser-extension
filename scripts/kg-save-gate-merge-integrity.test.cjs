...
test('gh pr merge blocks a merge commit that dropped an observation', () => {
  ...
  const result = runGate(dir, 'gh pr merge 12 --repo The-Doxa-Way/doxa-browser-extension --squash', ghbin);
...
// DIFFERENT ("real doxa-browser-extension") repo than the one the landing command
...
test('a landing command targeting a SIBLING repo is never blocked by that sibling\'s own merge history', () => {
  ...
  git(['init', '-q', '-b', 'main'], projectDir); // stands in for the real doxa-browser-extension checkout
